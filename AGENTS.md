...
**Related, separate bug found and fixed in the same notification:** the tilted level-3 card's "Level: N" text badge is baked into the element's `innerHTML` once, by `plantCardBody`, at PLANTING time (`Level: 0`) — the sliding-reveal `data-level` attribute update that happens on every growth step only drives the *on-planter* CSS animation, it doesn't touch that baked-in text, so the badge stayed frozen at "Level: 0" forever once the card left the planter (the tilted view has no planter background to slide against, so the text badge is its *only* level indicator, unlike a still-planted card). **General lesson: swapping CSS classes on a DOM element to change its visual treatment does not refresh any text that was baked into that element's `innerHTML` at an earlier render — if the new treatment needs different text, regenerate it explicitly** (`el.innerHTML = plantCardBody(...).inner`), the same render call `setup()`'s own level-3 rendering already uses for a fresh page load.

Regression test: `tests/level3PlantGrowth.test.mjs` — drives the real `notif_plantGrown`/`computePlayerStats`/`PlantingPhase.highlightGardenPlantsForCost` through headless Chrome, confirms all three symptoms (wrong level label, undercounted player panel, ineligible as a sacrifice) reproduce against the pre-fix code and are fixed post-fix.
