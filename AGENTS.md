...
- For games requiring live intermediate scoring (e.g., recalculated at the end of each phase), a robust pattern is to define a dedicated `calculateAllScores()` method in `Game.php`. This method should compute scores, update them via `$this->playerScore->set($playerId, $score, null)` / `$this->playerScoreAux->set($playerId, $scoreAux, null)` — **never** a raw `DbQuery` UPDATE of `player_score`/`player_score_aux`; BGA's static analyzer ("Dry run build") flags direct writes to those two columns, since only the counter API is auditable by BGA's own tooling. Both counters need `initDb($playerIds, initialValue: 0)` in `setupNewGame()` before first use. Passing `null` as the third arg suppresses the counter's own built-in notification if you're already sending a richer custom one (e.g. one that also carries `handCounts`) — then fire that `updateScores` notification yourself, same as before. See https://trello.com/c/alsJctgg.
...
- [x] `giveExtraTime()` wired up 2026-07-13 (Trello OSTxMchb) — `Game::giveExtraTimeToAllPlayers()` called alongside `setAllPlayersMultiactive()` in all 4 `MULTIPLE_ACTIVE_PLAYER` states (the only states where a player is ever waiting to act). Still needs the live-table realtime-mode manual test below to confirm end-to-end.
...
- [x] Tooltips — `addTooltipHtml()` covers character power cards and plant cards (the non-self-explanatory ones); weather cards show their name as visible text on the card face; most panel icons have `title` attributes, except the character panel icon, which uses `addTooltipHtml()` to show the full-size card (see "Character Panel Icon" below).
- [x] **Dark mode tested live by Marty 2026-07-13 — mostly fine.** No custom `prefers-color-scheme` handling needed; BGA's own dark mode already flips the page's default text color, and the game's hardcoded light pastel card `background-color`s (`#ebf5fb`, `#fdf2e9`, etc.) turned out not to be a problem in practice. The one real issue found — the "\<Player\>'s Garden"/"Bonus Weather"/"Public Weather Cards" section headers going white-on-white — was fixed in Trello CUKgx2vL by pairing an explicit `color` with each container's own deliberately-light background (see point 5 in "CSS Sprites & Inline-Style Gotchas" below). Character selection and the draft-keep modal were already fine (the latter already paired `color: white` with its own dark background).
...
The first pass at this fix (7CO2tan1) only regenerated the badge inside `notif_plantGrown`'s `if (args.max_level)` branch — i.e. only for the transition where a plant graduates off its planter to Level 3 (at the time, still rendered as a tilted tile in the garden — see "Level 3 Plants Are Data-Only" below for why that's no longer true). That fixed the max-level case but left every intermediate growth step (0→1, 1→2, still on the planter) badge-frozen at "Level: 0", which is exactly the narrower bug Marty then reported separately as https://trello.com/c/UlEhJIr5. **Lesson: when a fix regenerates baked-in text/markup in response to a state change, make sure it covers every transition that changes the underlying value, not just the one transition the original bug report happened to describe** — grep for every place the value can change, not just the one you're staring at. Fixed by moving the regeneration to run unconditionally whenever the DOM element exists, immediately after the `data-level` update, before the `max_level`-specific branch.

Regression tests: `tests/level3PlantGrowth.test.mjs` — drives the real `notif_plantGrown`/`computePlayerStats`/`level3CardsByColumn`/`PlantingPhase.renderSacrificeModal` through headless Chrome, confirming the location_arg translation is correct for every downstream consumer (undercounted player panel, missing tooltip data, ineligible as a sacrifice) — updated for https://trello.com/c/xYfPLZuI when Level 3 rendering and the sacrifice-picker both changed, but still exercising the same 7CO2tan1 root-cause fix. `tests/intermediatePlantGrowthBadge.test.mjs` covers the narrower badge-only regression for the 0→1 and 1→2 transitions while still on a planter.

---

## Append-Only Shared Containers Need ONE Canonical Insertion Order Across Every Code Path

When two different code paths both append into the same DOM container — e.g. `setup()` on page load/reload vs. a live `notif_*` handler during play — insertion order (and therefore left-to-right visual order) is whatever order that specific path happened to call things in. If `setup()` and the live notification populate the container in a different order, the same end state renders differently depending on which path last built it, and it looks like a nondeterministic bug even though each path is individually correct. This originally bit the per-player garden row (`player-garden-planters-<id>`) when it held both planters and a claimed-character card, back when `setup()` and `notif_characterClaimed` disagreed on insertion order (https://trello.com/c/nBsWlxlT) — one player's character card sat on a different side than the other's depending on how the client last rendered.

**Pattern:** pick the notification handler's order as canonical (it's the one that fires during live play, so it's the order players actually see most), and make every other code path that populates the same container — most commonly `setup()` — match it exactly. This general lesson still applies to any append-only shared container (the garden row still holds planters + played Bonus Weather tiles); the original character-card example is moot now that claimed characters no longer render into the garden row at all — see "Character Panel Icon" below (https://trello.com/c/Zn3wKWxj), which replaced `tests/characterCardPlacement.test.mjs` with `tests/characterPanelIcon.test.mjs`.

---

## Character Panel Icon, Not a Full Card in the Garden Row

A claimed character used to render as a full-size card (`.plantopia-character-power-card`) appended into the player's garden row, alongside planters and played Bonus Weather tiles — eating into an append-only row that's supposed to be reserved for the player's actual plantings. Per https://trello.com/c/Zn3wKWxj (Splendor-style: a small icon next to the player, not the physical card), the claimed character is now a small icon (`.plantopia-character-icon`, 24×36) in the player panel, to the left of the hand count, with a tooltip (`addCharacterTooltip`) showing the full-size card on hover.

- **No new image asset.** `.plantopia-character-icon` reuses the exact same `character_powers.png` sprite as `.plantopia-character-power-card` — `background-size` percentages are relative to the element's OWN box, so a smaller container just shows a proportionally shrunk version of the same card art.
- **Rendering moved from a DOM-node-move to a data-driven re-render**, matching this codebase's general "resync from `gamedatas`, don't patch the DOM" philosophy (see the notification-timing races documented elsewhere in this file). The OLD `notif_characterClaimed`/`notif_characterReturned` literally moved the SAME `<div>` element between `#available-characters-container` and the garden row (`appendChild`) — that trick doesn't survive the icon living inside `renderPlayerPanel`'s template, which fully rebuilds its container's `innerHTML` on every stat refresh (bonus weather play, plant grow, etc.), destroying any node planted inside it. Both notification handlers now instead move the card's data between `gamedatas.availableCharacters` and `gamedatas.claimedCharacters`, then re-render the two affected spots from that data: `renderCharacters(...)` for the selection pool, `renderPlayerPanel(playerId)` for the icon.
- **The SetupDecisions "return your character" click target moved too** — it used to search `.character-card` elements inside the garden row; now it targets `#character-icon-<playerId>` in the player panel directly. The `onLeavingState` cleanup also had to stop resetting the icon's cursor to `default` (unlike the selection panel's cards, which go invisible when `#characters-panel` hides, the panel icon stays visible for the rest of the game — it now reverts to `help`, its normal hover-for-tooltip cue, instead).

Regression test: `tests/characterPanelIcon.test.mjs` — drives the real `renderPlayerPanel`/`renderCharacters`/`addCharacterTooltip`/`notif_characterClaimed`/`notif_characterReturned` through headless Chrome, confirming the icon appears/disappears correctly, is positioned before the hand-count icon, carries a tooltip, and that nothing lands in the garden row for a claimed character.

---

## Level 3 Plants Are Data-Only — No Garden Rendering, Tooltip + Modal Instead

Level 3 (Treevolved) plants used to graduate off their planter into a dedicated `player-garden-tilted-<id>` row, rendered as a 90°-rotated tile (`.level3-tilted`). Per https://trello.com/c/xYfPLZuI ("conserve vertical space in the playing area"), that row is gone entirely — `gamedatas.plantsLevel3` is still tracked exactly as before (still the ONLY place Level 3 plant data lives), but it's never turned into a DOM element in the garden. Two consumers replace what the visible tile used to provide:

- **Player panel tooltip.** The compact plant-counts table's Lv. 3 row cells (see "Character Panel Icon" above for the table's own history) each get a deterministic id (`level3CellId(playerId, columnIcon)`, e.g. `lv3-cell-7-adult_tree`) regardless of count. `renderPlayerPanel` computes `level3CardsByColumn(playerId)` — the actual Level 3 card objects, bucketed into the same 6 columns as `PLANT_COUNT_COLUMNS` via `this.getFamily(...)`/`isAdult`/`isBabyType` — and wires a tooltip (`addLevel3Tooltip`, listing card names) onto each cell's id AFTER `renderPlayerPanel`'s `innerHTML` is set (a tooltip needs its target node already in the DOM — same ordering constraint as the character icon tooltip above). This is the ONLY place a player can see which specific cards make up a Lv. 3 count now.
- **Sacrifice picker modal.** Planting a Treevolved (adult) plant requires sacrificing one qualifying plant, and the eligible pool always included BOTH still-on-planter candidates (levels 0-2, cost-permitting) AND Level 3 candidates. The old `highlightGardenPlantsForCost` highlighted+clicked candidates directly in the garden — which flatly stopped working for Level 3 candidates once they stopped rendering there (nothing to click). Rather than leave two different interaction styles for the same choice (click-in-garden for planter-level candidates, something-else for Level 3 ones), `PlantingPhase.renderSacrificeModal` replaces it entirely: a "Draw X Keep Y"-style modal (same shell/pattern as `renderDraftModal`, single-select instead of multi-select) listing every eligible candidate from BOTH sources, with a status-bar Confirm button that only appears once something's selected. Its caller (`updateStatusBar`'s Treevolved branch) still needs its own "Cancel" button available throughout, unlike `renderDraftModal`'s caller — so `renderSacrificeModal`'s own `updateConfirmButton` re-adds Cancel every time it clears the status bar, instead of only ever adding Confirm.

`notif_plantGrown`'s `if (args.max_level)` branch no longer re-parents the DOM element into a tilted row — it just `el.remove()`s it. The `location_arg` translation from 7CO2tan1 (see "Moving a Card Between `gamedatas` Collections" above) is unchanged and still load-bearing: both `level3CardsByColumn` and `renderSacrificeModal`'s candidate filter read `plantsLevel3` entries directly, so a broken translation would silently empty out the tooltip AND make Level 3 candidates disappear from the sacrifice modal — not just miscount the player panel, like before.

Regression test: `tests/level3PlantGrowth.test.mjs` (rewritten for this card, see its own header comment) drives `notif_plantGrown`, `computePlayerStats`, `level3CardsByColumn`, and `PlantingPhase.renderSacrificeModal` together through headless Chrome.
...
```
This formula is container-size-independent — the same CSS works for hand cards (120×180), planter slots (~140 wide), and any other card-shaped container. Drive the per-card position with an attribute selector so the CSS file is one base class + N flat lines:
```css
...
**5. Always pair an explicit `background` with an explicit `color` — never rely on the page's light/dark default.** BGA's dark mode flips the page's DEFAULT text color to something light; it has no way to know that a given element's hardcoded background is meant to stay light (or dark) regardless of mode. The "\<Player\>'s Garden"/"Bonus Weather"/"Public Weather Cards" section headers set an inline `background: rgba(255,255,255,0.8)` container with no explicit text color — legible in light mode (dark default text), invisible in dark mode (light default text on a still-light background). Fixed by adding `color: #333` alongside. The draft-keep modal and the "Characters"/"My Hand" panels already did this correctly (either on the container or directly on the `<h3>`) and were unaffected. See https://trello.com/c/CUKgx2vL — when you hardcode ANY background color inline, hardcode its paired text color in the same breath, and don't assume "it looked fine" without checking both color schemes.
